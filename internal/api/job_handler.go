...
func (s *K8SServer) ListJobs(ctx context.Context, req *pb.NamespaceRequest) (*pb.WorkloadListResponse, error) {
	...
	if req == nil {
		reqStatus = "error"
		return nil, status.Error(codes.InvalidArgument, "job list request cannot be nil")
	}

	workloads, err := s.JobService.List(ctx, req.Namespace, req.LabelSelector, "")
	if err != nil {
		reqStatus = "error"
		s.Logger.Error("list jobs failed", "namespace", req.Namespace, "labelSelector", req.LabelSelector, "err", err)
		return nil, errorHelper(err, "job list")
